...
class Mechanism:
    ...
    def gaussian_noise_scale(self, l2_sensitivity, epsilon, delta):
        """ Return Gaussian sigma using local CDP accounting.

        For CDP, adding N(0, sigma^2) noise to an l2-sensitive query with sensitivity S
        yields rho = S^2 / (2 sigma^2). We compute rho that implies (epsilon, delta)-DP
        using util.rho_cdp.cdp_rho, then solve for sigma.
        """
        if self.bounded:
            l2_sensitivity *= 2.0
        rho = float(cdp_rho(float(epsilon), float(delta)))
        rho = max(rho, 1e-18)
        return l2_sensitivity / np.sqrt(2.0 * rho)
